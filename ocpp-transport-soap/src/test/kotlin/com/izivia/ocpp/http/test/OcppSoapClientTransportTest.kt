...
class OcppSoapClientTransportTest {
    ...
    fun `should send an OCPP 1-6 HeartBeat request`() {
        val server = createMockServerAndAlwaysRespond(HEARTBEAT_RESPONSE).start()
        ...
        expectThat(response) {
            get { currentTime }.isEqualTo(HEARTBEAT_TIME)
        }
    ...
    @Test
    fun `should advertise its callback url in the From header`() {
        val receivedFrom = mutableListOf<String?>()
        val server = createMockServerAndAlwaysRespond(HEARTBEAT_RESPONSE) { receivedFrom += it.from }.start()
        ...
        try {
            ...
            expectThat(receivedFrom).containsExactly("http://192.168.0.3:5001/ocpp/soap")
        } finally {
            client.close()
            server.stop()
        }
    ...
    @Test
    fun `should advertise the bound port when an ephemeral port is used`() {
        val receivedFrom = mutableListOf<String?>()
        val server = createMockServerAndAlwaysRespond(HEARTBEAT_RESPONSE) { receivedFrom += it.from }.start()
    ...
    @Test
    fun `should receive requests on the configured path`() {
        var received = false
        val client = OcppSoapClientTransport.createClient(
            SoapClientSettings(
                port = 5001,
        ...
        client.receiveMessage<HeartbeatReq, HeartbeatResp>("HeartBeat") {
            received = true
            HeartbeatResp(currentTime = HEARTBEAT_TIME)
        }

        try {
            client.connect()
            val result = JavaHttpClient().invoke(
                Request(Method.POST, "http://127.0.0.1:5001/ocpp/soap").body(HEARTBEAT_REQUEST)
            )

            expectThat(result.status).isEqualTo(Status.OK)
            expectThat(received).isTrue()
        } finally {
            client.close()
        }
    ...
    private fun createMockServerAndAlwaysRespond(
        handlerBody: String,
        onRequest: (RequestSoapMessage<Any>) -> Unit = {}
    ) =
        (
            "api" bind Method.POST to { request ->
                onRequest(Ocpp16SoapParser().parseAnyRequestFromSoap(request.bodyString()))
                Response(Status.OK).body(handlerBody)
            }
            )
            .asServer(Undertow(5002))

    companion object {
        private val HEARTBEAT_TIME = Instant.parse("2022-05-17T15:42:00.503Z")

        private val HEARTBEAT_RESPONSE = """
            <soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
                <soap:Header>
                    <Action xmlns="http://www.w3.org/2005/08/addressing">/HeartbeatResponse</Action>
                    <MessageID xmlns="http://www.w3.org/2005/08/addressing">urn:uuid:b5a704a7-a917-4ed3-bb8f-9a59918f8ffd</MessageID>
                ...
                <soap:Body>
                    <heartbeatResponse xmlns="urn://Ocpp/Cs/2015/10/">
                        <currentTime>$HEARTBEAT_TIME</currentTime>
                    </heartbeatResponse>
                </soap:Body>
            </soap:Envelope>
        """.trimIndent()
